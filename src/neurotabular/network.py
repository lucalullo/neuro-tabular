...
def _enforce_embedding_budget(
    widths: list[int], cardinalities: Sequence[int], memory_budget: int
) -> list[int]:
    """Reduce widths deterministically so their aggregate respects the budget."""

    if memory_budget < len(widths):
        raise ValueError(
            "embedding_memory_budget must provide at least one dimension per "
            "categorical feature."
        )
    allocated = list(widths)
    while sum(allocated) > memory_budget:
        candidates = [index for index, width in enumerate(allocated) if width > 1]
        index = max(
            candidates,
            key=lambda item: (allocated[item], cardinalities[item], -item),
        )
        allocated[index] -= 1
    return allocated
...
class TabularNetwork(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.embedding_dimensions = _enforce_embedding_budget(
            proposed_embedding_dimensions,
            categorical_cardinalities,
            embedding_memory_budget,
        )
