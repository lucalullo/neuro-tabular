...
class TabularPreprocessor:
    ...
    def fit(self, X: pd.DataFrame) -> TabularPreprocessor:
        ...
        for column in self.categorical_features_:
            ...
            try:
                ...
                # pandas.Categorical preserves declared-but-unobserved levels and
                # value_counts() reports them with count zero. Treat only values
                # actually observed in training as known categories.
    ...
    def _stabilize_knots(knots: np.ndarray) -> np.ndarray:
        ...
        return TabularPreprocessor._safe_float32(stable, "Numerical quantiles")

    @staticmethod
    def _safe_float32(values: np.ndarray, name: str) -> np.ndarray:
        """Convert finite values without silently creating infinities."""
    ...
    def _numeric_matrix(self, X: pd.DataFrame) -> np.ndarray:
        ...
        if np.iscomplexobj(converted.to_numpy(copy=False)):
            bad_columns = [
                column
                for column in self.numeric_features_
                if np.iscomplexobj(converted[column].to_numpy(copy=False))
            ]
            raise ValueError(
                "Numerical features must be real-valued; complex values are "
                f"not supported in columns: {bad_columns!r}."
            )
        try:
            values = converted.to_numpy(dtype=np.float64, na_value=np.nan, copy=True)
        except (TypeError, ValueError) as exc:
            raise ValueError(
                "A feature treated as numerical could not be converted to float64."
            ) from exc
