...
def test_aggregate_embedding_budget_is_enforced():
    cardinalities = [100] * 80
    model = TabularNetwork(
        ...
        n_blocks=1,
        dropout=0.0,
        embedding_memory_budget=128,
    )
    assert sum(model.embedding_dimensions) <= 128
    assert min(model.embedding_dimensions) >= 1
